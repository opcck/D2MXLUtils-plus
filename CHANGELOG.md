...
- **彻底剥离外部机器码 Hook**：移除向 D2Sigma 游戏代码段写入裸汇编 Trampoline 的危险注入逻辑，彻底恢复暗黑2进程的纯净状态，从根源上永久终结鼠标移至怪物或物品时的 `c0000005 (ACCESS_VIOLATION)` 崩溃。
- **重构为安全只读 Overlay 悬浮 HUD**：
  - 通过纯只读内存扫描（`pSelectedUnit`）实时读取光标所指怪物的 6 系抗性（物理、魔法、火焰、闪电、冰霜、毒素）、当前/最大生命值、百分比与 Class ID。
  - 通过现有的只读悬停捕获机制实时读取装备/地表物品的 UID、CID、孔数（`4s`）与无形状态（`eth`）。
  - 在前端透明 Overlay 视窗上新增 `<InspectorHud />` 悬浮卡片，带高颜值抗性色块与免疫高亮，彻底与游戏执行解耦，实现 100% 稳定运行。
