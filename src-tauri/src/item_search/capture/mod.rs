...
struct HookState {
    ...
    /// `remote_io::ProcessRef`, reused here rather than redefining an
    /// identical per-OS split.
    process: crate::remote_io::ProcessRef,
    d2sigma_base: usize,
    region: usize,
    region_size: usize,
    fields_addr: usize,
...
impl HoveredItemHook {
    ...
    pub(crate) fn install(
        ...
        // same tradeoff `injection/mod.rs`'s `RemoteAlloc` already makes).
        #[cfg(target_os = "linux")]
        let region = {
    ...
    pub(crate) fn uninstall(&self) -> Result<(), String> {
        ...
        #[cfg(target_os = "linux")]
        let restore_result =
            crate::remote_io::write_remote(state.process, target, &state.saved_bytes);
        ...
        // tradeoff as `dps/hook/linux.rs`/`notifier/visibility/hook`.
...
fn write_remote_with_page_protection(
    ...
    let write_result = crate::remote_io::write_remote(process, addr, data);
...
fn read_snapshot_fields(
    ...
    crate::remote_io::read_remote(process, fields_addr, &mut buf)
...
/// `dps/hook/linux.rs`'s `install` does.
#[cfg(target_os = "linux")]
...
pub(super) fn read_hovered_item_name(
    ...
    // Windows / Wine's own on Linux) — see `tick_clock::now_ms`'s doc
