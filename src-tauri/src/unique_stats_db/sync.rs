//! Keeps `app_data_dir/unique-stats-db.json` (see `unique_stats_db/storage.rs`) in
...
//! so `updater/release.rs`'s own release scan skips right over it (it fails that
...
//! loads `unique_stats_db/mod.rs`'s DB once into a plain (non-`RwLock`) field
...
fn fetch_release_asset() -> Result<GithubAsset, String> {
    ...
    let response = agent()
        ...
        // ACCEPT header updater/install.rs sets on its own GitHub calls.
